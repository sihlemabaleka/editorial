...
public class CropEditorialDialog extends DialogFragment {

	private CropImageView profilePictureCrop;
	private Bitmap croppedImage;
	private Button btnCrop;

	public static CropEditorialDialog newInstance(ArrayList<String> value, int position) {
		// TODO Auto-generated method stub
		CropEditorialDialog fragment = new CropEditorialDialog();
		Bundle args = new Bundle();
		args.putStringArrayList("files", value);
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		String filePath = getArguments().getStringArrayList("files").get(getArguments().getInt("position"));
	...
	class SaveProfilePhoto extends AsyncTask<Void, Integer, byte[]> {
		...
		protected void onPostExecute(byte[] data) {
			...
			if (data != null) {
				if(getArguments().getInt("position") != (getArguments().getStringArrayList("files").size() - 1)){
